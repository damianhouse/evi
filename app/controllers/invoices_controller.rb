class InvoicesController < ApplicationController
  ...
  def auto_create
    @appointments = Appointment.where('complete = ? OR no_show = ? AND invoice_id IS NULL', true, true).where.not(user_id: nil)
    @users = @appointments.map {|x| User.find(x.user_id)}
    @users = @users.uniq
    @invoices = []
    @users.each do |user|
      appointments = Appointment.where('user_id = ?', user.id).where('complete = ? AND invoice_id IS NULL OR no_show = ? AND invoice_id IS NULL' ,true, true)
  ...
  def create
    ...
    appointments = appointments = Appointment.where('user_id = ?', user.id).where('start_time >= ? AND end_time <= ? AND complete = ? OR start_time >= ? AND end_time <= ? AND no_show = ?', @invoice.start_date, @invoice.end_date, true, @invoice.start_date, @invoice.end_date, true)
